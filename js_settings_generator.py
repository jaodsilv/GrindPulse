...
def generate_js_settings():
    ...
    function readSettingsFromInputs() {
      ...
      // Base rate
      AWARENESS_CONFIG.baseRate = parseFloat(document.getElementById('setting-baseRate').value) || 2.0;

      // Solved count scaling
      AWARENESS_CONFIG.baseSolvedScaling = parseFloat(document.getElementById('setting-baseSolvedScaling').value) || 0.1;

      // Tier solved bonus
      AWARENESS_CONFIG.tierSolvedBonus.top = parseFloat(document.getElementById('setting-tierBonus-top').value) || 0.3;
      AWARENESS_CONFIG.tierSolvedBonus.advanced = parseFloat(document.getElementById('setting-tierBonus-advanced').value) || 0.2;
      AWARENESS_CONFIG.tierSolvedBonus.intermediate = parseFloat(document.getElementById('setting-tierBonus-intermediate').value) || 0.1;
      AWARENESS_CONFIG.tierSolvedBonus.below = parseFloat(document.getElementById('setting-tierBonus-below').value) || 0;
      ...
      tiers.forEach(tier => {
        difficulties.forEach(diff => {
          const input = document.getElementById(`setting-tdm-${tier}-${diff}`);
          if (input) {
            AWARENESS_CONFIG.tierDifficultyMultipliers[tier][diff] = parseFloat(input.value) || defaultMatrix[tier][diff];
          }
        });
      });

      // Thresholds
      AWARENESS_CONFIG.thresholds.white = parseInt(document.getElementById('setting-threshold-white').value) || 10;
      AWARENESS_CONFIG.thresholds.green = parseInt(document.getElementById('setting-threshold-green').value) || 30;
      AWARENESS_CONFIG.thresholds.yellow = parseInt(document.getElementById('setting-threshold-yellow').value) || 50;
      AWARENESS_CONFIG.thresholds.red = parseInt(document.getElementById('setting-threshold-red').value) || 70;
      AWARENESS_CONFIG.thresholds.darkRed = parseInt(document.getElementById('setting-threshold-darkRed').value) || 90;
    }
