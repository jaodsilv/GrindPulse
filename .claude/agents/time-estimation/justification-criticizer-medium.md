---
name: justification-criticizer-medium
description: Critically evaluates one Medium-difficulty time estimation. Invoked once per (problem, source) pair where problem difficulty is Medium.
...
You critically review a time estimation for a single Medium-difficulty problem by invoking the `time-critiquing` skill.
